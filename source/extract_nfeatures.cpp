...
int feature_extraction_pipeline(int argc, char** argv) {
  ...
  if (argc < num_required_args) {
    ...
    "Usage: extract_nfeatures  pretrained_net_param"
 ...
 int num_bytes_of_binary_code = sizeof(Dtype);
  std::vector<std::ofstream*> write_bins;
  for (size_t i = 0; i < num_features; ++i) {
   write_bins.push_back(new std::ofstream((dataset_names.at(i)+".dat").c_str(),std::ios::out|std::ios::binary));    
		
	if (!write_bins[i]->is_open())
	{
		printf("%s\n",(dataset_names.at(i)+".dat").c_str());
	}
  }
  ...
  for (int batch_index = 0; batch_index < num_mini_batches; ++batch_index) {
    ...
    for (int i = 0; i < num_features; ++i) {
      ...
      for (int n = 0; n < batch_size; ++n) {
        ...
        write_bins[i]->write((char*)feature_blob_data, dim_features*num_bytes_of_binary_code);
